...
def validate_signal_length(data, min_length=3000):
    return len(data) >= min_length

def trim_signal_symmetric(data, target_length=3000):
...
def extract_welch_features(signal_data, fs=50, nperseg=256):
    try:
...
def process_single_file(file_path, output_dir_spectral, output_dir_wavelets, dataset_id):
    try:
        ...
        if any(col not in df.columns for col in required):
            ...
            return False
        if not validate_signal_length(df,3000):
            print(f"Too short: {file_path.name}")
            return False
        trimmed = trim_signal_symmetric(df,3000)
        amp = trimmed['AMP'].values
        md = {k: trimmed[k].iloc[0] for k in ['DIST','Angle','ON','Movement','Timestamp']}
...
def main():
    ...
    clean_root = base_dir / 'dataset_clean'
    pre_root = base_dir / 'dataset_preprocessed'
    dataset_id = 0
    for sub in sorted(clean_root.iterdir()):
        if not sub.is_dir():
            continue
        for f in sorted(sub.glob('clean_data_*.csv')):
            ...
            if len(name_parts) < 4:
                continue
            sample_id = name_parts[2]
            sample_folder = pre_root / f"dataset_sample_{sample_id}"
